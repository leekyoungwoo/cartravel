...
if os.environ.get('CARTRAVEL_MOD') == 'development':
    sys.path.append('C:\\workspace\\cartravel\\api\\src')
else:
    sys.path.append('/CARTRAVEL/api')
...
if not cur.fetchone():
    ...
    f = open(os.path.join(GLOBAL_CONFIG.CARTRAVEL_ROOT_DIR, 'migrate', 'cartravel_init.dump'), encoding='utf-8')
