...
export function timeDiffFromUTCEpoch(epochUTC: any): string {
  ...
  if (absDiff !== 0) {
    if (absDiff < 61) {
      // 초단위 차이로 처리
      diff = String(parseInt(String(absDiff), 10)) + '초';
    } else if (absDiff < 3601) {
      // 분단위 차이
      diff = String(parseInt(String(absDiff / 60), 10)) + '분';
    } else if (absDiff < 86401) {
      // 시간단위 차이
      diff = String(parseInt(String(absDiff / 3600), 10)) + '시간';
    } else if (absDiff < 2592001) {
      // 일단위 차이
      diff = String(parseInt(String(absDiff / 86400), 10)) + '일';
    } else if (absDiff < 31536001) {
      // 월단위 차이
      diff = String(parseInt(String(absDiff / 2592000), 10)) + '개월';
    } else {
      // 년단위 차이
      diff = String(parseInt(String(absDiff / 31536000), 10)) + '년';
    }
  } else {
    // 현재
    diff = '지금';
